...
SPAG <- function(companiesDF, shp, theoreticalSample=1000, empiricalSample=1000, numberOfSamples=1, columnAreaName,
  ...
  if(class(shp) %in% c("SpatialPoints")){
    pointsAsPolygon <- list(Polygon(shp@coords))
    polygonAsPolygons <- Polygons(lista, c(1))
    listOfPolygons <- list(lista_sp)
    shp <- SpatialPolygons(new_list, proj4string = shp@proj4string)
  }
